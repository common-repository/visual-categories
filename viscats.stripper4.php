...
if (!class_exists('strip_attributes')) {
	...
	*/
	...
	define('STRIP_COMMENTS', 1);
	define('STRIP_ALL_COMMENTS', 2);
	...
	class strip_attributes {
		...
		function find_attributes($nodes) {
		...
		function remove_attributes($nodes) {
		...
		function is_exception($element_name, $attribute_name, $exceptions) {
		...
		function create_attributes($new_attributes, $name, $value) {
			if($new_attributes)
				$new_attributes .= " ";
